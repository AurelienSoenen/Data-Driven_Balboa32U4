...
T_values = (100:50:1400); % Generates values 10 to 80
rng(50);
T = 1500;
...
for t = 1:T-1
    ...
    rdm_state_measurement = [randn(m,1); randn(m,1); randn(m,1); randn(m,1)];
    rdm_state_measurement = rdm_state_measurement*0.016;
    x(:,t+1) = A * x(:,t) + B * u(:,t) + rdm_state_measurement; % Give same gain with Bessel or ITAE
end

% Kalman filter initialization
Q = eye(n) * 0.001;  % Process noise covariance
R = eye(n) * 0.0005;  % Measurement noise covariance
P = eye(n);  % Initial estimate error covariance
x_hat = zeros(n, T);  % Estimated states
x_hat(:, 1) = x(:, 1);
...
for k = 2:T
    % Prediction step
    x_hat(:, k) = A_KF * x_hat(:, k-1) + B * u(:, k-1);
    P = A_KF * P * A_KF' + Q;

    % Update step
    K = P * eye(n) / (eye(n) * P * eye(n)' + R);
    x_hat(:, k) = x_hat(:, k) + K * (x(:, k) - eye(n) * x_hat(:, k));
    P = (eye(n) - K * eye(n)) * P;
end
...
for T = T_values
    ...
    X_0 = x_filtered(:,1:T-1); % Dimension of X_0 : n x T-1
    X_1 = x_filtered(:,2:T); % Dimension of X_1 : n x T-1
...
% Write to CSV
file = 'WithExcitementVariableKF2';
