...
for rdmGain = rdmGains
    ...
    for t = 1:T-1
        u(:,t) = - K_itae_theory * x(:,t) + randn(m,1)*rdmGain;
    ...
    X_0(1,:) = min(max(X_0(1,:), -pi/2), pi/2);
    X_1(1,:) = min(max(X_1(1,:), -pi/2), pi/2);
...
plot(rdmGains, eig_diff1_values, 'k-o', 'LineWidth', 1.25, 'MarkerFaceColor', 'k');
hold on;
plot(rdmGains, eig_diff2_values, 'k--o', 'LineWidth', 1.25, 'MarkerFaceColor', 'w');

% Adding labels
xlabel('Excitement std [V]');
ylabel('MRE [-]');
...
% Save the figure
saveas(gcf, fullfile(folderName, 'ExcitementAnalysis.png'));
