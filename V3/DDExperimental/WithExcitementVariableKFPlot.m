...
filePath = sprintf('../data/KF_rdmITAE1.csv');
...
for T = T_values
    ...
    eig_diff1 = mean(abs(abs(K_directDD - K_theory)./abs(K_theory)));
    %eig_diff1 = mean(abs(abs(K_directDD - K_theory)));
    ...
    if T > 5
        ...
        K_indirectDD = K_indirectDD;
        eig_diff2 = mean(abs(abs(K_indirectDD - K_theory)./-K_theory));
        %eig_diff2 = mean(abs(abs(K_indirectDD - K_theory)));
...
% Write to CSV
file = 'NoExcitementVariable5Plot';
